<template>
  <div class="comm-body">
    ...
    <div class="addSite clearfix">
      <x-button type="warn" link="/addSite/0" style="background-color: #39bafc; font-size: 16px;">
        <x-icon type="ios-plus-outline" size="20" style="position: relative;top: 4px;"></x-icon>
        新增
      </x-button>
    </div>
    <ul class="container">
      <li v-if="siteList.length === 0" style="border-bottom: none; text-align: center;">数据为空</li>
      <li class="clearfix" v-else v-for="(item) in siteList" :key="item.id">
        <div>
          {{item.f_name}}
          <p>
        ...
        <div>
          ...
          <div class="clearfix container-btn">
            <x-button type="primary" mini @click.native="downLine(item.id)" style="background-color:#39bafc;">编辑</x-button>
          </div>
        </div>
      </li>
    </ul>
